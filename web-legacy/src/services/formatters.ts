...
// have to guard.
export function formatEventDateTime(date: string | null | undefined, time: string | null | undefined): { date: string; time: string } {
  const out = { date: '—', time: '' }
  if (!date) return out
  ...
  if (d) {
    out.date = d.toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' }).replace('.', '')
  } else {
    out.date = date
  }
