...
interface WithdrawalResponse {
  ...
  checkedIn?: boolean
  checkedInAt?: string
  /** True when the scan also triggered a check-in for a participant who had
   * not been validated at the entrance yet. Surfaced so the scanner UI can
   * mention that the check-in happened implicitly along with the retirada. */
  implicitCheckIn?: boolean
  kit?: {
    items: Array<{ label: string; variant: string | null }>
    affected: number
    // Per-item failures surfaced by the scanner endpoint so the operator
    // knows that (e.g.) the medalha didn't leave stock even though the
    // camiseta did. Empty when everything succeeded. Code 'KIT_WITHDRAWN_PARTIAL'
    // on the outer response flags the partial case.
...
/**
 * Register kit delivery for a participant. Hits /api/mobile/checkin with
 * mode='withdrawal', which runs the server-side inventory reservation and
 * returns 409 if the kit was already picked up.
 */
